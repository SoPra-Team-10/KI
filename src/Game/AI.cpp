...
double evalBludgers(const std::shared_ptr<gameModel::Environment> env){
    ...
    for(int i = 0; i < 2; i++){
        ...
        //eval team1
        if(team1->keeper->position != bPos) val -= 10 / (gameController::getDistance(bPos, team1->keeper->position) + 1);
        if(team1->seeker->position != bPos) val -= 20 / (gameController::getDistance(bPos, team1->seeker->position) + 1);

        for(int j = 0; j < 3; j++){
            if(team1->chasers[j]->position != bPos) val -= 10 / (gameController::getDistance(bPos, team1->chasers[j]->position) + 1);
        }
        for(int j = 0; j < 2; j++){
            if(team1->beaters[j]->position != bPos) val += 20 / (gameController::getDistance(bPos, team1->beaters[j]->position) + 1);
        }

        //eval team2
        if(team2->keeper->position != bPos) val += 10 / (gameController::getDistance(bPos, team2->keeper->position) + 1);
        if(team2->seeker->position != bPos) val += 20 / (gameController::getDistance(bPos, team2->seeker->position) + 1);

        for(int j = 0; j < 3; j++){
            if(team2->chasers[j]->position != bPos) val += 10 / (gameController::getDistance(bPos, team2->chasers[j]->position) + 1);
        }
        for(int j = 0; j < 2; j++){
            if(team2->beaters[j]->position != bPos) val -= 20 / (gameController::getDistance(bPos, team2->beaters[j]->position) + 1);
        }
    }
    return val;
}
